 ...
 */
export default class ProxyInterface extends EventEmitter {
  /**
    ...
    if (Array.isArray(xml.method)) {
      for (const m of xml.method) {
        ...
        const wrapper = function wrapper (...args) {
          return object._callMethod(name, method.name, method.inSignature, method.outSignature, ...args);
        };

        wrapper.begin = function (...args) {
          object._beginMethod(name, method.name, method.inSignature, method.outSignature, ...args);
        };

        iface[identifierToJs(method.name)] = wrapper;
      }
    }
